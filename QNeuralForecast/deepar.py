...
class QDeepAR(BaseModel):
    """QDeepAR

    Quantum-enhanced DeepAR model that replaces the classical MLP decoder with a
    QuantumProjection layer. The architecture uses an LSTM encoder to process time series
    data, followed by a quantum variational circuit for the final projection to distribution
    parameters.

    **Quantum Modification:**
    The classical MLP decoder is replaced with QuantumProjection:
    - pre_net: Linear(lstm_hidden_size → n_qubits)
    - q_layer: PennyLane quantum circuit with variational parameters
    - post_net: Linear(n_qubits → distribution_parameters)
    ...
    Args:
        ...
        n_qubits (int): Number of qubits in the quantum circuit. Default: 4.
        n_qlayers (int): Number of variational layers in the quantum circuit. Default: 1.
        circuit_device (str): PennyLane device name. Options:
            - "default.qubit": Ideal quantum simulator (no noise)
            - "default.mixed": Noisy quantum simulator (NISQ-realistic)
    ...
    def __init__(
        ...
        n_qubits: int = 4,
        n_qlayers: int = 1,
        circuit_device: str = "default.qubit",
        ...
        **trainer_kwargs
        ...
        # ============================================================
        # QUANTUM MODIFICATION: Replace classical MLP decoder with QuantumProjection
        # ============================================================
        # Classical DeepAR uses:
        # self.decoder = MLP(
        ...
        # )
        #
        # QDeepAR replaces it with QuantumProjection:
        self.decoder = QuantumProjection(
            input_size=lstm_hidden_size,
            output_size=self.loss.outputsize_multiplier,
            n_qubits=n_qubits,
            n_layers=n_qlayers,
    ...
    def forward(self, windows_batch):
        ...
        # ============================================================
        # QUANTUM MODIFICATION: Quantum decoder replaces classical MLP
        # ============================================================
        # Classical: output = MLP(hidden_state)
        # Quantum: output = QuantumProjection(hidden_state)
